...
const ShowerTypeManager = () => {
  ...
  useEffect(() => {
    axios.get(`${API}/shower-types`).then(r => setTypes(r.data));
  }, [refresh]);

  const save = async () => {
    if (!name) return;
    if (editing) {
      await axios.put(`${API}/shower-types/${editing.id}`, { name });
    } else {
      await axios.post(`${API}/shower-types`, { name });
    }
    setEditing(null);
    setName("");
    setRefresh(r => r + 1);
  ...
  const del = async (id) => {
    if (window.confirm("Delete this shower type?")) {
      await axios.delete(`${API}/shower-types/${id}`);
      setRefresh(r => r + 1);
    }
  };
  ...
  return (
    <div>
      <h2 className="font-bold mb-2">Shower Types</h2>
        ...
        <tbody>
          {types.map(t => (
            <tr key={t.id}>
              <td>{t.name}</td>
              <td>
                <button className="text-xs text-blue-600" onClick={() => { setEditing(t); setName(t.name); }}>Edit</button>
                {" "}
